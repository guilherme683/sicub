...
export type EntityResponseType = HttpResponse<Servidores>;

@Injectable()
export class ServidoresService {

    private resourceUrl =  SERVER_API_URL + 'api/servidores';
    private resourceSearchUrl = SERVER_API_URL + 'api/_search/servidores';
    ...
    private convertResponse(res: EntityResponseType): EntityResponseType {
        const body: Servidores = this.convertItemFromServer(res.body);
        return res.clone({body});
    }

    private convertArrayResponse(res: HttpResponse<Servidores[]>): HttpResponse<Servidores[]> {
        const jsonResponse: Servidores[] = res.body;
        const body: Servidores[] = [];
    ...
    /**
     * Convert a returned JSON object to Servidores.
     */
    private convertItemFromServer(servidores: Servidores): Servidores {
        const copy: Servidores = Object.assign({}, servidores);
        return copy;
    }

    /**
     * Convert a Servidores to a JSON which can be sent to the server.
     */
    private convert(servidores: Servidores): Servidores {
        const copy: Servidores = Object.assign({}, servidores);
        return copy;
    }
}
